...
import {
  ...
  NameChanged, PubkeyChanged, Resolver, ResolverVersions, TextChanged, VersionChanged
} from './types/schema';
import { createResolverID, createResolverVersionsID, getOrCreateResolver, getResolverID } from "./utils";
...
export function handleNameChanged(event: NameChangedEvent): void {
  ...
  resolverEvent.resolver = getResolverID(event.params.node, event.address)
  resolverEvent.blockNumber = event.block.number.toI32()
  resolverEvent.transactionID = event.transaction.hash
...
export function handleVersionChanged(event: RecordVersionChanged): void {
  ...
  let resolver = Resolver.load(createResolverID(event.params.node, event.address, event.params.newVersion))
  if (resolver == null) {
    resolverEvent.isNewVersion = true
    resolver = getOrCreateResolver(event.params.node, event.address, event.params.newVersion)
  }
  resolverEvent.save()
