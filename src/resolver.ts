...
export function handleAddrChanged(event: AddrChangedEvent): void {
  ...
  let name = Name.load(event.params.node.toHexString());
  if (name && name.resolver == resolver.id) {
    name.resolvedAddress = event.params.a.toHexString();
    name.save();
  }
...
function getOrCreateResolver(node: Bytes, address: Address): Resolver {
  ...
  if (resolver === null) {
    ...
    resolver.name = node.toHexString();
